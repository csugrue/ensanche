	...
	if( best_error_ndx != -1 ) {
		for( i=0; i<cursize; i++ ) {
			...
			if( blobs[i].id != -1 ) {
				...
				float deltaLocx = (blobs[i].centroid.x - oldblob->centroid.x);
				float deltaLocy = (blobs[i].centroid.y - oldblob->centroid.y);
				float deltLocSq = deltaLocx+deltaLocy;
				float deltaArea = blobs[i].area - oldblob->area;
				if( deltaArea > .25*oldblob->area)  blobs[i].id = -1;
			}
